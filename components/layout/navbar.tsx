    ...
    <>
      <AnimatePresence>
        {!scrolled ? (
          ...
          <motion.nav
            key="sticky-pill"
            className="fixed top-4 left-4 right-4 md:left-0 md:right-0 md:mx-auto md:w-fit z-50 rounded-full bg-[var(--sticky-nav-bg)] backdrop-blur-md border border-[var(--sticky-nav-border)] p-2 shadow-lg"
            initial={{ y: -100, opacity: 0 }}
            animate={{ y: 0, opacity: 1 }}
            exit={{ y: -100, opacity: 0 }}
            transition={{ duration: 0.2 }}
            ...
            <div className="flex items-center justify-between md:justify-center gap-4 px-2">
                    ...
                    >
                      ...
                      {hoveredPath === item.href ? (
                        <motion.span
                          ...
                          className="absolute inset-0 bg-background border border-black/5 dark:border-white/10 rounded-full shadow-sm -z-10"
                          transition={{
                            type: "spring",
                            stiffness: 300,
                            damping: 30
              ...
              <div className="flex items-center gap-2">
                ...
                <Button 
                  asChild
                  size="sm" 
                  className="hidden md:inline-flex bg-foreground text-background hover:bg-foreground/90 rounded-full px-4 h-9 shadow-md"
                >
                  <Link href="/contact">Get In Touch</Link>
                </Button>
                ...
                <Button
                  ...
                  size="icon"
                  onClick={() => setIsOpen(!isOpen)}
                  className="md:hidden h-9 w-9 rounded-full bg-black/5 dark:bg-white/5"
                >
